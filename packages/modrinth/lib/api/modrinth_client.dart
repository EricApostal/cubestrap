...
import 'versions/versions_manager.dart';
import 'version_files/version_files_manager.dart';
import 'users/users_manager.dart';
import 'notifications/notifications_manager.dart';
import 'threads/threads_manager.dart';
import 'teams/teams_manager.dart';
import 'tags/tags_manager.dart';
import 'misc/misc_manager.dart';

/// Labrinth `vv2.7.0/366f528`.
///
...
class ModrinthClient {
  ...
  static String get version => 'v2.7.0/366f528';

  ProjectsManager? _projects;
  VersionsManager? _versions;
  VersionFilesManager? _versionFiles;
  UsersManager? _users;
  NotificationsManager? _notifications;
  ThreadsManager? _threads;
  TeamsManager? _teams;
  TagsManager? _tags;
  MiscManager? _misc;

  ProjectsManager get projects => _projects ??= ProjectsManager(_dio, baseUrl: _baseUrl);

  VersionsManager get versions => _versions ??= VersionsManager(_dio, baseUrl: _baseUrl);

  VersionFilesManager get versionFiles => _versionFiles ??= VersionFilesManager(_dio, baseUrl: _baseUrl);

  UsersManager get users => _users ??= UsersManager(_dio, baseUrl: _baseUrl);

  NotificationsManager get notifications => _notifications ??= NotificationsManager(_dio, baseUrl: _baseUrl);

  ThreadsManager get threads => _threads ??= ThreadsManager(_dio, baseUrl: _baseUrl);

  TeamsManager get teams => _teams ??= TeamsManager(_dio, baseUrl: _baseUrl);

  TagsManager get tags => _tags ??= TagsManager(_dio, baseUrl: _baseUrl);

  MiscManager get misc => _misc ??= MiscManager(_dio, baseUrl: _baseUrl);
}
